...
def signup_patient(request):
    ...
    if request.method == 'POST':
      ...
      if request.POST['username'] and request.POST['email'] and  request.POST['name'] and request.POST['password']and request.POST['password1'] :
          ...
          if password == password1:
              if User.objects.filter(username = username).exists():
                ...
                patientnew = patient(user=user,name=name)
                patientnew.save()
...
def signup_doctor(request):
    ...
    if request.method == 'POST':
      ...
      if request.POST['username'] and request.POST['email'] and  request.POST['name']  and request.POST['password']and request.POST['password1']  and  request.POST['registration_no'] and request.POST['qualification'] and  request.POST['State_Medical_Council'] and  request.POST['specialization'] :
          ...
          if password == password1:
              if User.objects.filter(username = username).exists():
                ...
                doctornew = doctor( user=user, name=name, registration_no=registration_no, qualification=qualification, State_Medical_Council=State_Medical_Council, specialization=specialization )
                doctornew.save()
                messages.info(request,'user created sucessfully')
                print("doctorcreated")
